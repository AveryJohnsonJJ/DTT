...
#             skeleton = np.einsum('abcd,kd->abck', skeleton, matrix_x)
            joint_bottom = skeleton[0, 0, self.zaxis[0],:3]
            joint_top = skeleton[0, 0, self.zaxis[1],:3]
            axis = np.cross(joint_top - joint_bottom, [0, 0, 1])
            angle = self.angle_between(joint_top - joint_bottom, [0, 0, 1])
            matrix_z = self.rotation_matrix(axis, angle)
            new_skeleton = np.einsum('abcd,kd->abck', skeleton[:,:,:,:3], matrix_z) 
            skeleton = np.concatenate((new_skeleton, skeleton[:,:,:,3:]), axis=3) 
            joint_rshoulder = skeleton[0, 0, self.xaxis[0],:3]
            joint_lshoulder = skeleton[0, 0, self.xaxis[1],:3]
            axis = np.cross(joint_rshoulder - joint_lshoulder, [1, 0, 0])
            angle = self.angle_between(joint_rshoulder - joint_lshoulder, [1, 0, 0])
            matrix_x = self.rotation_matrix(axis, angle)
            new_skeleton = np.einsum('abcd,kd->abck', skeleton[:,:,:,:3], matrix_x) 
            skeleton = np.concatenate((new_skeleton, skeleton[:,:,:,3:]), axis=3) 
